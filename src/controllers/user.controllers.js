...
const generateAccessAndRefereshTokens = async (userId) => {
    try {
        ...
        console.log(error)
        throw new ApiError(500, "Something went wrong while genrateing Refresh and Access token")
    }
}
...
const updateUserAvatar = asyncHandler(async (req, res) => {
    ...
    try {
        const avatarDeleted = await User.findByIdAndUpdate(
            req.user?._id,
            { $unset: { avatar: "" } }, // Use $unset to remove the field
            { new: true }
        );

        if (avatarDeleted) {
            new ApiResponse(200, "Avatar removed successfully")
        }
    } catch (error) {
